...
Public Class fgol_per
    ...
    Private Sub opendata()
        ...
        Const sql As String = "SELECT  top 100   ms_gol_tt.kode,ms_golongan.kode as kd_gol, ms_golongan.nama, ms_gol_tt.tanggal, ms_gol_tt.tanggal1, ms_gol_tt.tanggal2, ms_gol_tt.ket " & _
            "FROM ms_gol_tt INNER JOIN ms_golongan ON ms_gol_tt.kd_gol = ms_golongan.kode"
    ...
    Private Sub opendata2(ByVal vkode As String)
        ...
        Dim sql As String = String.Format("SELECT  ms_karyawan.nip, ms_karyawan.nama " & _
            "FROM  ms_gol_tt2 INNER JOIN ms_karyawan ON ms_gol_tt2.nip = ms_karyawan.nip  where ms_gol_tt2.kode='{0}'", vkode)
    ...
    Private Sub cari()
        ...
        Dim sql2 As String = "SELECT  top 300 ms_gol_tt.kode,ms_golongan.kode as kd_gol, ms_golongan.nama, ms_gol_tt.tanggal, ms_gol_tt.tanggal1, ms_gol_tt.tanggal2, ms_gol_tt.ket " & _
            "FROM ms_gol_tt INNER JOIN ms_golongan ON ms_gol_tt.kd_gol = ms_golongan.kode"

        Select Case tcbofind.SelectedIndex
            Case 0 ' kode
                sql2 = String.Format("{0} where ms_gol_tt.kode like '%{1}%'", sql2, tfind.Text.Trim)
            Case 1 ' tanggal
                ...
                sql2 = String.Format("{0} where ms_gol_tt.tanggal='{1}'", sql2, convert_date_to_eng(tfind.Text.Trim))

            Case 2 ' ket
                sql2 = String.Format("{0} where ms_gol_tt.ket like '%{1}%'", sql2, tfind.Text.Trim)
            Case 3 ' nip
                sql2 = String.Format("{0} where ms_gol_tt.kode in (select kode from ms_gol_tt2 where nip='{1}')", sql2, tfind.Text.Trim)
        End Select
    ...
    Private Sub hapus()
        ...
        Dim sql As String = String.Format("delete from ms_gol_tt where kode='{0}'", dv1(Me.BindingContext(bs1).Position)("kode").ToString)
        Dim sql2 As String = String.Format("delete from ms_gol_tt2 where kode='{0}'", dv1(Me.BindingContext(bs1).Position)("kode").ToString)
        ...
        Try
            ...
            Clsmy.InsertToLog(cn, "btgol_per", 0, 0, 1, 0, dv1(Me.BindingContext(bs1).Position)("kode").ToString, dv1(Me.BindingContext(bs1).Position)("ket").ToString, sqltrans)
    ...
    Private Sub tsedit_Click(sender As System.Object, e As System.EventArgs) Handles tsedit.Click
        ...
        Using fjamker2 As New fgol_per2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = False, .position = bs1.Position}
            fjamker2.ShowDialog()
        End Using
    ...
    Private Sub tsadd_Click(sender As System.Object, e As System.EventArgs) Handles tsadd.Click
        Using fjamker2 As New fgol_per2 With {.StartPosition = FormStartPosition.CenterParent, .dv = dv1, .addstat = True, .position = 0}
            fjamker2.ShowDialog()
        End Using
    End Sub
