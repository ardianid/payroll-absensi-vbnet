...
Public Class fbon2
    ...
    Private Function isi_nobukti(ByVal cn As OleDbConnection, ByVal sqltrans As OleDbTransaction) As String
        ...
        Dim nobuktiaw As String = "BNK." & bulan & Year(Date.Now).ToString.Substring(2, 2)

        Dim sql As String = String.Format("select MAX(nobukti) from ms_bon where nobukti like '{0}%'", nobuktiaw)
    ...
    Private Sub simpan()
        ...
        Try
            ...
            Dim sqlins As String = String.Format("insert into ms_bon (nobukti,tanggal,nip,jumlah,ket) values('{0}','{1}','{2}',{3},'{4}')", tbukti.Text.Trim, convert_date_to_eng(ttgl.EditValue), tnip.Text.Trim, Replace(tjumlah.EditValue, ",", "."), tket.Text.Trim)
            Dim sqlup As String = String.Format("update ms_bon set tanggal='{0}',nip='{1}',jumlah={2},ket='{3}' where nobukti='{4}'", convert_date_to_eng(ttgl.EditValue), tnip.Text.Trim, Replace(tjumlah.EditValue, ",", "."), tket.Text.Trim, tbukti.Text.Trim)
